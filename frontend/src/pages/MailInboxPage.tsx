...
function validationStatusLabel(t: (k: string) => string, status: string) {
  ...
  if (status === "SENT_WITHOUT_VALIDATION")
    return t("mailValidationStatusSentNoVal");
  return status;
}
...
export function MailInboxPage({
  ...
  const pages = useMemo(
    () => Math.max(1, Math.ceil(listTotal / 20)),
    [listTotal],
  );
  ...
  const loadThreads = useCallback(
    async (p: number) => {
      ...
      const res =
        mode === "admin"
          ? await api.adminMailThreads(token, opts)
          : await api.muniMailThreads(token, opts);
  ...
  const refreshList = useCallback(
    async (p = 1) => {
      ...
      try {
        ...
        setError(
          formatApiErrorMessage(e instanceof Error ? e.message : String(e), t),
        );
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="card">
      ...
      <div
        className="row mailInboxTabs"
        style={{ gap: 8, marginBottom: 10, flexWrap: "wrap" }}
      >
        <button
          type="button"
          className={`btn ${tab === "inbox" ? "btnPrimary" : ""}`}
      ...
      {tab === "inbox" ? (
        <div className="mailList">
          {(threads || []).map((th) => {
            const muniLabel = th.recipient_municipality
              ? isFr
            ...
            return (
              <Link
                key={th.id}
                to={`/mail/${th.id}`}
                className={`mailRow ${th.unread ? "mailRowUnread" : ""}`}
              >
                <div style={{ display: "grid", gap: 2 }}>
                  <div style={{ fontWeight: th.unread ? 900 : 700 }}>
                    {muniLabel ? (
                      <span className="mailMuni">{muniLabel} — </span>
                    ) : null}
                    {th.subject}
                  </div>
                  <div className="row" style={{ gap: 6, flexWrap: "wrap" }}>
                    ...
                    {th.validation_outcome === "VALIDATED" ? (
                      <span className="statusPill stOk">
                        {t("mailValidatedBadge")}
                      </span>
                    ) : null}
                    {th.validation_outcome === "SENT_WITHOUT_VALIDATION" ? (
                      <span className="statusPill stNever">
                        {t("mailSentWithoutValidationBadge")}
                      </span>
                    ) : null}
                  </div>
                </div>
                {th.unread ? (
                  <div className="statusPill stOut">{t("mailUnread")}</div>
                ) : null}
              </Link>
            );
          })}
        ...
        <div className="mailList">
            ...
            >
              ...
              <div
                className={`statusPill ${
                  vr.status === "CHANGES_REQUESTED"
                    ? "stNever"
                    : vr.my_validator_decision === "PENDING"
                      ? "stOut"
                      : "stOk"
                }`}
              >
          ...
          {!loading && !validations?.length ? (
            <div className="muted">
              {validationView === "validator"
                ? t("mailValidationEmptyValidator")
                : t("mailValidationEmptyAuthor")}
            </div>
          ) : null}
        </div>
      )}
